export const StarsReview: React.FC<{ rating: number, size: number }> = (props) => {
    ...
    if (rating !== undefined && rating > 0 && rating <= 5) {
        //iterate
        for (let i = 0; i <= 4; i++) {
            //subtract until u get to .5 stars; once you get that rating -.5 will become neg, move to next elif
            if (rating - 1 >= 0) {
    ...
    return (
        // array.from creates an array of given length of stars, second argument loops through and maps it to the star
        <div>
            {Array.from({ length: fullStars }, (_, i) =>
                // width and height is passed in, style = color
                <svg key={i} 
